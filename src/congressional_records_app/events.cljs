(ns congressional-records-app.events
  (:require
   [congressional-records-app.db    :refer [default-db todos->local-store]]
   [re-frame.core :refer [reg-event-db reg-event-fx inject-cofx path after]]
   [cljs.spec.alpha :as s]
   [ajax.core :as ajax]
   [congressional-records-app.secrets :as secrets]))
...
(def check-spec-interceptor (after (partial check-and-throw :congressional-records-app.db/db)))
...
(reg-event-fx
 :handler-with-http
 (fn [{:keys [db]} _]
   {;:db (assoc db :show-twirly true) ;; NOTE: can use this for status indicator
    :http-xhrio {:method :get
                 :uri "https://api.open.fec.gov/v1/candidates/"
                 :params {:api_key secrets/soopersecret}
                 :timeout 8000
...
(reg-event-db
 :good-http-result
 (fn [db [_ result]]
   (assoc db :good-http-result result)))
